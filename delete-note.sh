...
OUTPUT=`curl -X POST -s -H "Content-Type: application/json" -d '{"userId": "'$USERID'","i": "'$TOKEN'","limit": 100}' https://miss.nem.one/api/users/notes`
...
do
    ...
    OUTPUT=`curl -s -X POST -H "Content-Type: application/json" -d '{"userId": "'$USERID'","i": "'$TOKEN'","untilId": "'$UNTIL_ID'","limit": 100}' https://miss.nem.one/api/users/notes`
    CREATED_AT+=(`echo $OUTPUT | jq -r '.[] | .createdAt'`)
    NOTE_ID+=(`echo $OUTPUT | jq -r '.[] | .id'`)
...
for n in "${CREATED_AT[@]}"
do
    if [ $(($CURRENT_TIME_UNIX - $n)) -ge 3600 ]; then
        sleep 1
